...
desc 'Pull, parse new calendar and add to database'
task :calpull => :environment do
  calPath = String.new
  ...
  htmldoc.xpath('//td/small/a').each do |tag|
    if tag.text.include? 'iCal'
      calPath = "http://timeedit.ita.chalmers.se#{tag.attribute('href')}"
    end
  end
  cal = Vpim::Icalendar.decode(RestClient.get calPath).first
  Calendar.create!(:cal_dump => Marshal.dump(cal))
end
